...
async function run() {
    ...
    // Integration with extensions on automatic capture solution
...
async function checkAll(first) {
    ...
    if (document.URL.match(/vk.com\/*/)) {
        // TODO need to completely rewrite all logic here for the new VK interface
        ...
        chrome.runtime.sendMessage({errorAuthVK: 'Most likely your VK account has been blocked, if this is not the case and this error occurs constantly, please report to the extension developer!'})
        return
    }
    ...
    if (document.URL.match(/discord.com\/*/)) {
        ...
        if (document.location.search.includes('%20guilds.join') || document.location.search.includes('%20guilds') || document.location.search.includes('+guilds.join') || document.location.search.includes('+guilds') || document.location.search.includes('%20email') || document.location.search.includes('+email') || !document.location.search.includes('prompt=none')) {
            ...
            //Remedy for greediness in permissions
            ...
            //Force authorization without bothering the user if permissions were already granted
    ...
    if (document.querySelector('#challenge-form') || document.querySelector('#challenge-body-text')) {
        //If we need to click "Verify you are human" https://gyazo.com/56426c80a3072e5b4d565949af7da81b
    ...
    // TODO CloudFlare returns an empty page and tells the browser that everything is loaded. All that is known is what is specified in the selector below, conflicts with sites (monitors) are possible
    ...
    // If error 5xx https://i.imgur.com/aO5H1k8.png
    ...
    // If error (access forbidden) CloudFlare https://i.imgur.com/u3kPXYL.png (or any other error, there's nothing to do querySelector for)
...
// TODO return at least some result to background when executeScript to avoid the error "Could not establish connection. Receiving end does not exist"
